...
def visualizeTrees(newick_list, node_weights_list, input_trees, all_node_weights, figsize=(12, 12)):
    """
...
def all_possible_clusters(tree, distance_matrix):
    tree_leaves = sorted([leaf.name for leaf in tree.get_leaves()], key=lambda x: int(re.search(r'\d+', x).group()))
    tree_leaves_with_inds = {obj: i for i, obj in enumerate(tree_leaves)}
    internal_nodes = 0
    ...
    best_sil_score = -1
    best_nc = 2
    for nc in range(2, internal_nodes+1):
        labels = np.zeros(len(tree_leaves), dtype=int)
        clusters = hierclust(tree, nc)
        all_clusters[nc] = clusters
        for cluster_id, objects in clusters.items():
            for obj in objects:
                labels[tree_leaves_with_inds[obj]] = cluster_id
        sil_score = silhouette_score(distance_matrix, labels, metric='precomputed')
        if sil_score > best_sil_score:
            best_sil_score = sil_score
            best_nc = nc
    return best_nc, all_clusters, internal_nodes


def plot_with_ete3(ete_tree, all_clusters, internal_nodes, num_clusters):
    if num_clusters > internal_nodes:
        clusters = all_clusters[internal_nodes]
    else:
        clusters = all_clusters[num_clusters]
    k = len(clusters)
    ts = TreeStyle()
...
def nj_clustering_with_outgroup(distance_matrix, output_cluster_file, out_dist):
    ...
    best_nc, all_clusters, internal_nodes = all_possible_clusters(ete_tree, distance_matrix)
    with open(output_cluster_file, "w") as file:
        for key, subdict in all_clusters.items():
            file.write(f"{key} clusters:\n")
            for subkey, value in subdict.items():
                file.write(f"{subkey}:\t{', '.join(value)}\n")
            file.write("------------------\n")
    return ete_tree, best_nc, all_clusters, internal_nodes
...
def miniclust(folder, output_dismat_file, output_cluster_file, b = 0):
    ...
    ete_tree, best_nc, all_clusters, internal_nodes = nj_clustering_with_outgroup(normalized_distance_mat, output_cluster_file, out_dist=1.01)
    return input_trees, all_node_weights, new_trees, new_trees_nodes_weights, ete_tree, all_clusters, best_nc, internal_nodes


def show_all_trees(new_trees, new_trees_nodes_weights, input_trees, all_node_weights):
    width, height = get_monitor_sizes()
    fig_all = visualizeTrees(new_trees, new_trees_nodes_weights, input_trees, all_node_weights, figsize = (width, height))
